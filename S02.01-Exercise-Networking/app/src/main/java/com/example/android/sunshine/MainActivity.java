...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadWeatherData(){
        String prefLoc = SunshinePreferences.PREF_CITY_NAME;
        URL builtURL = NetworkUtils.buildUrl(prefLoc);
        WeatherTask data = new WeatherTask();
        data.execute(builtURL);
    }
    ...
    private class WeatherTask extends AsyncTask<URL, Void, String>{
        ...
        protected String doInBackground(URL... urls){
            try {
                String results = NetworkUtils.getResponseFromHttpUrl(urls[0]);
                return results;
            }
            catch (IOException e){
                e.printStackTrace();
